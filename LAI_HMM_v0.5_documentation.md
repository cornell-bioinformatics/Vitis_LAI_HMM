...
`LAI_HMM_v0.5.py` supports four workflows:
...
- A FASTA index file ending in `.fai` or `.fai.gz`, where column 1 is chromosome and column 2 is length (additional columns are optional; file should be headerless).
- A .csv/.tsv delimited table with chromosome name and chromosome length columns (header optional).

Example .fai:
```text
chr01   27822162        7       27822162        27822163
chr02   20941263        27822177        20941263        20941264
chr03   21317290        48763448        21317290        21317291
chr04   25934928        70080746        25934928        25934929
```
...
| `--legend-title STR` | Title used for the chromosome painting legend. Default: `Clade`. |
| `--refgv STR` | Optional genome reference version label to record in outputs and append to the plot footer when provided. |
| `--refsetv STR` | Optional population/reference-profile version label to record in outputs and append to the plot footer when provided. |
...
- `figures/<sample>_<date>.png`: chromosome painting figure.
  The legend title defaults to `Clade` and can be changed with `--legend-title`.
  The footer always includes `model: LAI_HMM_v0.5`, and appends `refgv=<value>` and/or `refsetv=<value>` when those optional version labels are supplied.
...
```bash
python LAI_HMM_v0.5.py ... --lam-per-Mb 0.02
```
...
```bash
python LAI_HMM_v0.5.py ... --b0 0.5 --no-plots
```
...
module_path = Path("LAI_HMM_v0.5.py")
