...
def _open_vcf_reader(vcf_path: str, sample_names: Optional[Sequence[str]] = None) -> Any:
    """Open a cyvcf2 VCF reader with an optional sample subset."""
    if VCF is None:
        raise ImportError("cyvcf2 is required for VCF input. Install it with `pip install cyvcf2`.")
    if sample_names is None:
        return VCF(vcf_path, gts012=True)
    return VCF(vcf_path, gts012=True, samples=list(sample_names))


def _header_contigs_for_iteration(vcf: Any) -> List[str]:
    """
    Return usable header contigs for region-based iteration.

    Some VCFs have placeholder contigs such as `NA` in the headers or are missing contig info. 
    In those cases we avoid trusting the header and fall back to sequential record iteration.
    """
    seqnames = [str(c).strip() for c in getattr(vcf, "seqnames", []) if str(c).strip()]
    if not seqnames:
        return []
    bad = {"NA", ".", "None"}
    if all(c in bad for c in seqnames):
        return []
    return seqnames


def _selected_vcf_contigs(vcf: Any, contig_whitelist: Optional[Set[str]] = None) -> Optional[List[str]]:
    """
    Choose contigs for record iteration.

    Returns a list when region-based iteration should be attempted, or `None` when
    we should scan the file sequentially and optionally filter by `contig_whitelist`.
    """
    header_contigs = _header_contigs_for_iteration(vcf)
    if not header_contigs:
        return None if contig_whitelist is None else sorted(str(c) for c in contig_whitelist)
    if contig_whitelist is not None:
        selected = [c for c in header_contigs if c in contig_whitelist]
        return selected if selected else None
    return header_contigs


def _iter_vcf_records(vcf: Any, selected_contigs: Optional[Sequence[str]] = None) -> Iterable[Any]:
    """
    Iterate VCF records robustly.

    We prefer region-based iteration when usable contigs are available, but fall
    back to sequential iteration for plain/unindexed VCFs or VCFs with malformed
    contig headers.
    """
    if selected_contigs:
        try:
    ...
    allowed = set(str(c) for c in selected_contigs) if selected_contigs is not None else None
    for rec in vcf:
        if allowed is None or str(rec.CHROM) in allowed:
            yield rec
...
def load_vcf_chunk_matrix(
    ...
    vcf = _open_vcf_reader(vcf_path, sample_names=sample_names)
    samples = list(vcf.samples)
    if not samples:
        vcf.close(); raise ValueError("VCF has no samples.")
    N = len(samples)

    contigs = _selected_vcf_contigs(vcf, contig_whitelist=contig_whitelist)
    use_marker_annotations = require_marker or vcf_has_any_marker_annotations(
        vcf_path, contigs, sample_names=sample_names
    )
    vcf.close()

    # -------- Pass 1: enumerate loci and build row maps (robust to ordering) --------
    loci: List[Tuple[str,int,str,str,str,List[str]]] = []
    # per-record chromosome: map (pos, id_or_marker) -> row index
    rowmap: Dict[str, Dict[Tuple[int, str], int]] = defaultdict(dict)

    unnamed_marker_count = 0
    vcf_pass1 = _open_vcf_reader(vcf_path, sample_names=sample_names)
    try:
        for rec in _iter_vcf_records(vcf_pass1, contigs):
    ...
    vcf_pass2 = _open_vcf_reader(vcf_path, sample_names=sample_names)
    try:
        for rec in _iter_vcf_records(vcf_pass2, contigs):
            marker = resolve_vcf_marker_name(
                rec,
                require_marker=require_marker,
                use_marker_annotations=use_marker_annotations,
            ...
            row = rowmap[str(rec.CHROM)].get(key)
            if row is None:
...
def harmonize_variant_markers_with_marker_positions(
    variants: List[VCFVariant],
    marker_df: pd.DataFrame,
) -> Tuple[List[VCFVariant], Dict[str, int]]:
    """
    Remap per-sample VCF variant markers to the marker_positions names by exact site.

    This is mainly for VCF-only workflows where INFO/MARKER is absent and the VCF
    fallback label (`CHROM:POS`) differs from the user-supplied marker table name
    (for example `Chr01:147572` vs `Chr01_147572`).
    """
    if not variants:
        return [], {"kept": 0, "remapped": 0, "ambiguous": 0, "unmatched": 0}

    marker_norm = normalize_marker_positions(marker_df)
    valid_names = {str(m).strip() for m in marker_norm["marker"]}
    site_to_markers: Dict[Tuple[str, int], List[str]] = defaultdict(list)
    for row in marker_norm[["marker", "chrom", "pos"]].itertuples(index=False):
        site_to_markers[variant_site_key(row.chrom, row.pos)].append(str(row.marker).strip())

    out: List[VCFVariant] = []
    stats = {"kept": 0, "remapped": 0, "ambiguous": 0, "unmatched": 0}
    for v in variants:
        current_marker = str(v.marker).strip() if v.marker is not None else ""
        if current_marker in valid_names:
            out.append(v)
            stats["kept"] += 1
            continue

        matches = site_to_markers.get(variant_site_key(v.chrom, v.pos), [])
        if len(matches) == 1:
            out.append(replace(v, marker=matches[0]))
            stats["remapped"] += 1
        else:
            out.append(v)
            if len(matches) > 1:
...
def _vcf_marker_alignment_summary(
    marker_df: pd.DataFrame,
    chunk: VCFChunk,
) -> Dict[str, Any]:
    """Summarize exact marker-ID and chrom+pos alignment between marker_positions and a VCF chunk."""
    marker_norm = normalize_marker_positions(marker_df)
    marker_names = {str(m).strip() for m in marker_norm["marker"]}
    marker_sites = {
        variant_site_key(row.chrom, row.pos)
        for row in marker_norm[["chrom", "pos"]].itertuples(index=False)
    }

    loci = chunk.locus_df[["chrom", "pos", "marker"]].drop_duplicates().copy()
    loci["marker"] = loci["marker"].astype(str).str.strip()
    vcf_markers = set(loci["marker"])
    vcf_sites = {
        variant_site_key(row.chrom, row.pos)
        for row in loci[["chrom", "pos"]].itertuples(index=False)
    }
    fallback_labels = bool(
        (loci["marker"] == (loci["chrom"].astype(str) + ":" + loci["pos"].astype(int).astype(str))).all()
    ) if not loci.empty else False

    return {
        "marker_total": int(len(marker_names)),
        "vcf_marker_total": int(len(vcf_markers)),
        "exact_marker_overlap": int(len(marker_names & vcf_markers)),
        "site_overlap": int(len(marker_sites & vcf_sites)),
        "all_vcf_markers_are_site_labels": fallback_labels,
    }


def _warn_marker_alignment(
    *,
    marker_df: pd.DataFrame,
    chunk: Optional[VCFChunk] = None,
    hap_gt: Optional[pd.DataFrame] = None,
    allow_vcf_site_fallback: bool = False,
    verbose: bool = True,
) -> None:
    """Print one-time warnings about marker-ID mismatches across inputs."""
    if not verbose:
        return
    ...
    if chunk is not None:
        summary = _vcf_marker_alignment_summary(marker_norm, chunk)
        exact = summary["exact_marker_overlap"]
        total = summary["vcf_marker_total"]
        site_overlap = summary["site_overlap"]
        if exact < total:
            if summary["all_vcf_markers_are_site_labels"] and allow_vcf_site_fallback and site_overlap > 0:
                print(
                    "Warning: marker_positions marker IDs do not exactly match the VCF site labels "
                    f"({exact}/{total} exact marker-ID overlaps). "
                    f"Using chrom+pos fallback for {site_overlap} VCF site(s) in this VCF-only run."
                )
            else:
                print(
                    "Warning: marker_positions marker IDs do not fully match the VCF markers "
                    f"({exact}/{total} exact marker-ID overlaps"
                    + (f"; {site_overlap}/{total} chrom+pos overlaps" if site_overlap else "")
                    + ")."
                )

    if hap_gt is not None:
        hap_markers = {str(m).strip() for m in hap_gt.index}
        hap_overlap = len(marker_names & hap_markers)
        if hap_overlap < len(hap_markers) or hap_overlap < len(marker_names):
            print(
                "Warning: marker_positions marker IDs do not fully match the hap_genotype markers "
                f"({hap_overlap}/{len(hap_markers)} hap_genotype marker IDs overlap exactly)."
            )
...
def vcf_has_any_marker_annotations(
    vcf_path: str,
    ...
    vcf = _open_vcf_reader(vcf_path, sample_names=sample_names)
    try:
        for rec in _iter_vcf_records(vcf, contigs):
...
def calculate_variant_reference_frequencies(
    ...
    vcf = _open_vcf_reader(vcf_path, sample_names=ref_samples)
    sample_clades = [sample_clade[s] for s in vcf.samples]
    contig_list = _selected_vcf_contigs(vcf, contig_whitelist=contigs)
    use_marker_annotations = require_marker or vcf_has_any_marker_annotations(
        vcf_path, contig_list, sample_names=ref_samples
    )
    rows = []
    freq_map: Dict[Tuple[str, int, str], np.ndarray] = {}

    try:
        for rec in _iter_vcf_records(vcf, contig_list):
            marker = resolve_vcf_marker_name(
                rec,
                require_marker=require_marker,
                use_marker_annotations=use_marker_annotations,
            ...
            for ix, allele in enumerate(alleles):
                ...
                for c in clades:
                    ...
                    vec.append(float(f))
                rows.append(item)
                freq_map[variant_allele_key(rec.CHROM, rec.POS, allele)] = np.asarray(vec, dtype=float)
    finally:
        vcf.close()
...
def _vcf_sample_feature_matrix(vcf_path: str, samples: Sequence[str], contigs: Optional[Set[str]] = None) -> np.ndarray:
    ...
    vcf = _open_vcf_reader(vcf_path, sample_names=samples)
    rows = []
    try:
        contig_list = _selected_vcf_contigs(vcf, contig_whitelist=contigs)
        for rec in _iter_vcf_records(vcf, contig_list):
...
def load_chrom_lengths(path: str, sep: Optional[str] = None) -> pd.DataFrame:
    ...
    if lower.endswith((".fai", ".fai.gz")):
        ...
        table_sep = _guess_table_sep(path, sep)
        chrom_aliases = {"chrom", "chr", "chromosome"}
        pos_aliases = {"pos", "length", "bp", "end"}

        df = pd.read_csv(path, sep=table_sep, dtype=str)
        lower_to_col = {str(c).strip().lower(): c for c in df.columns}
        has_named_cols = (
            any(a in lower_to_col for a in chrom_aliases)
            and any(a in lower_to_col for a in pos_aliases)
        )
        ...
        if not has_named_cols:
            ...
            chrom_col = next(lower_to_col[a] for a in ("chrom", "chr", "chromosome") if a in lower_to_col)
            pos_col = next(lower_to_col[a] for a in ("pos", "length", "bp", "end") if a in lower_to_col)
...
def run_unordered_hmm_from_vcf(
    ...
    if variants:
        variants, marker_map_stats = harmonize_variant_markers_with_marker_positions(variants, marker_df)
        if debug and (marker_map_stats["remapped"] or marker_map_stats["ambiguous"] or marker_map_stats["unmatched"]):
            print(
                "VCF marker reconciliation against marker_positions: "
                f"{marker_map_stats['remapped']} remapped by chrom+pos, "
                f"{marker_map_stats['kept']} already matched, "
                f"{marker_map_stats['ambiguous']} ambiguous, "
                f"{marker_map_stats['unmatched']} unmatched"
            )
    clades = _validate_clades(clades or DEFAULT_CLADES)
    ...
    return {
        ...
        "mus_hap_alleles_used": mus_hap_alleles is not None,
...
def _build_params_dict(kwargs: Dict[str, Any], outdir: str, clades: Sequence[str]) -> Dict[str, Any]:
    ...
    params.setdefault("legend_title", "Clade")
    params.setdefault("refgv", "NA")
    params.setdefault("modelv", "0.5")
    params.setdefault("refsetv", "NA")
    return params


def _version_footer_label(params: Dict[str, Any]) -> str:
    """Build the plot footer version label from optional user-supplied version tags."""
    modelv = str(params.get("modelv", "NA")).strip() or "NA"
    parts = [f"model: LAI_HMM_v{modelv}"]

    refgv = str(params.get("refgv", "")).strip()
    if refgv and refgv.upper() != "NA":
        parts.append(f"refgv={refgv}")

    refsetv = str(params.get("refsetv", "")).strip()
    if refsetv and refsetv.upper() != "NA":
        parts.append(f"refsetv={refsetv}")

    return " | ".join(parts)


def _reserve_run_log_path(outdir: str) -> Path:
    timestamp = time.strftime("%Y-%m-%d_%H-%M-%S")
    return Path(outdir) / f"run_{timestamp}.log"
...
@contextmanager
def _tee_console_to_log(logfile: Path) -> Iterable[None]:
    logfile.parent.mkdir(parents=True, exist_ok=True)
    with logfile.open("a", encoding="utf-8", buffering=1) as handle:
        sys.stdout.flush()
        sys.stderr.flush()
        ...
        def _forward(read_fd: int, target_fd: int) -> None:
            try:
                while True:
        ...
        out_thread = threading.Thread(target=_forward, args=(out_r, saved_stdout_fd), daemon=True)
        err_thread = threading.Thread(target=_forward, args=(err_r, saved_stderr_fd), daemon=True)
        out_thread.start()
        err_thread.start()
...
def _write_run_log(
    ...
    *,
    ...
    logfile = Path(logfile) if logfile is not None else _reserve_run_log_path(outdir)
    logfile.parent.mkdir(parents=True, exist_ok=True)
    timestamp = logfile.stem.replace("run_", "", 1) if logfile.stem.startswith("run_") else time.strftime("%Y-%m-%d_%H-%M-%S")
    payload = {
        "timestamp": timestamp,
...
def run_lai_hmm_for_sample(
    ...
    **hmm_kwargs
    ...
    params["mus_hap_alleles_used"] = bool(hmm_results.get("mus_hap_alleles_used"))
    ...
    **hmm_kwargs
    ...
    if write_log and save_outputs:
        logfile = _write_run_log(
            ...
            logfile=Path(logfile_path) if logfile_path is not None else None,
    ...
    if marker_positions_path:
        _warn_marker_alignment(
            marker_df=marker_df,
            chunk=chunk,
            hap_gt=hap_gt,
            allow_vcf_site_fallback=bool(vcf_path and not hap_genotype_path and not effective_require_marker),
...
def prepare_to_plot(hmm_result):
    ...
    chr20_adjustments_enabled = bool(hmm_result.get("chr20_adjustments_enabled")) and bool(hmm_result.get("mus_hap_alleles_used"))
...
def chromosome_painting(hap_long, clade_percentages, indv, params, chromlengths, markerticks=True, message=None, annotation=None, savefig=True, showplot=True):
    ...
    chr20_adjustment_active = (
        bool(p.get("mus_hap_alleles_used"))
        and bool(p.get("chr20_adjustments_enabled"))
        and bool(p.get("chr20presence"))
    )
    ...
    # Guard against empty data or samples where every marker is missing.
    if (not plottable) or bool(hap_long["MissingMarkers"].all()):
    ...
    fig, ax = plt.subplots(figsize=(18, max(3, len(chromosome_items) * 0.9)))
    ax.set_xlim(0, maxL)
    ...
    ax.set_title(f"Ancestry of {indv}", fontsize=20, pad=16)
    ...
    ax.legend(
        ...
        title=f"{p.get('legend_title', 'Clade')} (certainty = opacity)",
    ...
    version_label = _version_footer_label(p)
    fig.text(0.01, 0.032, # bottom left corner
             version_label, ha="left", va="bottom", fontsize=8)
    fig.tight_layout(rect=[0.03, 0.035, 0.89, 0.96])
...
Examples:
  Build reference files only:
    python LAI_HMM_v0.5.py --step build-reference --hap-genotype example_data/hap_genotype_refset_example.gz --vcf example_data/example_refset.vcf.gz --reference-membership example_data/example_reference_membership.tsv --clades EA,Mus,NA,Vv --reference-outdir demo_reference --pca both --downsample 0.5

  VCF only:
    python LAI_HMM_v0.5.py --vcf example_data/example_samples.vcf.gz --variant-profiles example_data/reference_variant_profiles.tsv --marker-positions example_data/marker_positions.csv --no-plots --sample SAMPLE1

  hap_genotype only:
    python LAI_HMM_v0.5.py --hap-genotype example_data/hap_genotype_samples_example --hap-frequencies example_data/reference_hap_allele_frequency_lookup.pkl --marker-positions example_data/marker_positions.csv --chrom-lengths example_data/chrom_lengths.fai --all-samples

  combined VCF + hap_genotype, four worker threads:
    python LAI_HMM_v0.5.py --vcf example_data/example_samples.vcf.gz --hap-genotype example_data/hap_genotype_samples_example --variant-profiles example_data/reference_variant_profiles.tsv --hap-frequencies example_data/reference_hap_allele_frequency_lookup.pkl --hap-informativeness example_data/reference_hap_allele_informativeness.tsv --marker-positions example_data/marker_positions.csv --chrom-lengths example_data/chrom_lengths.fai --all-samples --threads 4
""",
    )
    ...
    samples.add_argument("--legend-title", "--legend_title", dest="legend_title", default="Clade", help="Legend title used on chromosome painting figures. Default: Clade.")
    samples.add_argument("--refgv", default="NA", help="Optional reference genome version label to record in outputs and plot footers.")
    samples.add_argument("--refsetv", "--refsetv", dest="refsetv", default="NA", help="Optional reference population version number to record in outputs and plot footers.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    logfile_path = _reserve_run_log_path(args.outdir) if args.save_outputs else None

    def _run_cli_hmm() -> int:
        result = run_lai_hmm(
            vcf_path=args.vcf,
            hap_genotype_path=args.hap_genotype,
            ...
            logfile_path=str(logfile_path) if logfile_path is not None else None,
            command_text=command_text,
            refgv=args.refgv,
            refsetv=args.refsetv,
            verbose=args.verbose,
            debug=args.debug,
        ...
        print(f"Log file written to: {logfile_path}")
        if errors:
    ...
    if logfile_path is not None:
        with _tee_console_to_log(logfile_path):
            return _run_cli_hmm()
    return _run_cli_hmm()
